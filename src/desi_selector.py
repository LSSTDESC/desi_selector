...
class DesiSelector:
    ...
    def measure_autocorr(self, path_mock, path_rand, z_range=[0.8, 1.1], min_sep=1, max_sep=200, nbins=25):
    
    
    
    mock_cat_in = pd.read_parquet(path=path_mock)
    mask_mock_z_cut = np.logical_and(mock_cat_in['redshift_true'] > z_range[0], mock_cat_in['redshift_true'] < z_range[1]) 
    mock_cat = mock_cat_in[mask_mock_z_cut]

    rand_cat_in = pd.read_parquet(path=path_rand)
    mask_rand_z_cut = np.logical_and(rand_cat_in['redshift_true'] > z_range[0], rand_cat_in['redshift_true'] < z_range[1]) 
    rand_cat = rand_cat_in[mask_rand_z_cut]
